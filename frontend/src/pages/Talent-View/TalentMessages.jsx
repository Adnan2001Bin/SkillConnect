...
import { FaPaperPlane, FaTimes, FaCheck, FaCheckDouble, FaTrash } from "react-icons/fa";
...
const TalentMessages = () => {
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      ...
      try {
        const response = await fetch(
          `${import.meta.env.VITE_API_BASE_URL}/api/chat/${selectedChat._id}/messages`,
          {
            method: "GET",
            credentials: "include",
          }
  ...
  const handleDeleteMessage = (messageId) => {
    confirmToast("Are you sure you want to delete this message?", async () => {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_API_BASE_URL}/api/chat/${selectedChat._id}/message/${messageId}`,
          {
            method: "DELETE",
            credentials: "include",
          }
  ...
  return (
    <>
      ...
      >
        <div className="max-w-6xl mx-auto bg-very-light-teal rounded-xl shadow-md p-6 sm:p-8 flex flex-col lg:flex-row gap-6 h-[80vh]" style={{ backgroundColor: "#e8f0ef" }}>
          {/* Chat List */}
          <div className="w-full lg:w-1/3 bg-off-white rounded-lg p-4 overflow-y-auto" style={{ backgroundColor: "#f1f2f2" }}>
            <h2 className="text-xl font-semibold text-dark-gray mb-4" style={{ color: "#333" }}>
              Chats
            </h2>
            {chats.length === 0 ? (
              ...
              chats.map((chat) => (
                <motion.div
                  ...
                  className={`p-3 mb-2 rounded-lg cursor-pointer ${
                    selectedChat?._id === chat._id ? "bg-teal text-white" : "bg-white"
                  }`}
                  style={{
                    backgroundColor: selectedChat?._id === chat._id ? "#4da59b" : "#fff",
                  }}
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      <p className="font-medium text-sm">{getOtherParticipant(chat)}</p>
          ...
          {/* Chat Messages */}
          <div className="w-full lg:w-2/3 bg-off-white rounded-lg p-4 flex flex-col" style={{ backgroundColor: "#f1f2f2" }}>
            {selectedChat ? (
              <>
                <div className="flex justify-between items-center mb-4">
                  <h2 className="text-xl font-semibold text-dark-gray" style={{ color: "#333" }}>
                        ...
                        <div
                          className={`max-w-xs sm:max-w-md p-3 rounded-2xl shadow-sm ${
                            message.sender._id === user._id
                              ...
                              : "bg-gray-200 text-dark-gray rounded-bl-none"
                          }`}
                          style={{
                            backgroundColor:
                              message.sender._id === user._id ? "#4da59b" : "#e5e7eb",
                            color: message.sender._id === user._id ? "#fff" : "#333",
                          }}
                        >
                          <p>{message.content}</p>
                ...
                <form onSubmit={handleSendMessage} className="flex gap-2">
                  <input
                    ...
                    className="flex-1 px-4 py-2 border border-medium-teal rounded-lg focus:outline-none focus:ring-2 focus:ring-teal text-sm sm:text-base"
                    style={{ borderColor: "#98c2bd" }}
                  />
                  <button
                    type="submit"
                    disabled={isSending || !newMessage.trim()}
                    className="px-4 py-2 bg-teal text-white rounded-lg hover:bg-teal-dark disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-200 flex items-center gap-2"
